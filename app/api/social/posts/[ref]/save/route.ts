...
import { toggleSave } from "@/features/social/services/post.service";
...
export async function POST(_request: Request, context: { params: Promise<{ ref: string }> }) {
  ...
  try {
    return NextResponse.json(await toggleSave(session.user.id, ref));
  } catch (error) {
    return jsonSocialError(error, "Could not save post.");
  }
}
