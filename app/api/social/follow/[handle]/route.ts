...
import { toggleFollow } from "@/features/social/services/profile.service";
...
export async function POST(_request: Request, context: { params: Promise<{ handle: string }> }) {
  ...
  try {
    return NextResponse.json(await toggleFollow(session.user.id, handle));
  } catch (error) {
    return jsonSocialError(error, "Could not follow.");
  }
}
