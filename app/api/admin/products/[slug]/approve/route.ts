...
import { staffApproveProduct } from "@/features/shops/services/product-admin.service";
...
export async function POST(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const product = await staffApproveProduct(session.user.id, slug, ip);
    return NextResponse.json({
      slug: product.slug,
      moderationStatus: product.moderationStatus,
    });
  } catch (error) {
    return jsonShopError(error, "Could not approve listing.");
  }
}
