...
import { FavoriteError, toggleFavorite } from "@/features/marketplace/services/favorite.service";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await toggleFavorite(session.user.id, parsed.data.slug);
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof FavoriteError && error.code === "NOT_FOUND") {
