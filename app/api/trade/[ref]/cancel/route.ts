...
import { cancelTrade } from "@/features/trade/services/trade.service";
...
export async function POST(request: Request, context: { params: Promise<{ ref: string }> }) {
  ...
  const limited = await rateLimit(`trade-cancel:${session.user.id}`, 30, 15 * 60 * 1000);
  if (!limited.success) {
  ...
  try {
    const trade = await cancelTrade(session.user.id, ref, parsed.data, clientIp(request));
    return NextResponse.json(trade, { headers: noStore });
  } catch (error) {
    return jsonTradeError(error, "Could not cancel trade.");
  }
}
