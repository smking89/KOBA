...
import { rejectTrade } from "@/features/trade/services/trade.service";
...
export async function POST(request: Request, context: { params: Promise<{ ref: string }> }) {
  ...
  const limited = await rateLimit(`trade-reject:${session.user.id}`, 30, 15 * 60 * 1000);
  if (!limited.success) {
    return NextResponse.json(
      { error: "Too many reject attempts." },
  ...
  try {
    const trade = await rejectTrade(session.user.id, ref, parsed.data, clientIp(request));
    return NextResponse.json(trade, { headers: noStore });
  } catch (error) {
    return jsonTradeError(error, "Could not reject trade.");
  }
}
