...
import { changePasswordSchema } from "@/features/staff-mfa/schemas/staff-mfa.schemas";
import { changeStaffPassword } from "@/features/staff-mfa/services/staff-mfa.service";
...
export async function POST(request: Request) {
  ...
  const limited = await rateLimit(`mfa-password:${session.user.id}`, 5, 15 * 60 * 1000);
