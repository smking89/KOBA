...
2. **Hosting provider** — resolved (client, restated 2026-08-18 after this note went stale): a GoDaddy VPS, managed via Plesk. Not a managed platform (Vercel/Render/Fly) or a single AWS/GCP account as this note originally framed as the options — object storage, sharding, and deploy pipeline notes elsewhere in this doc should assume a self-managed VPS target, not a serverless/managed-platform one.
...
4. **Voice/video call provider** (Phase 6 DMs) — resolved (client, restated 2026-08-18): deferred entirely. Ship as a "Coming soon" placeholder in the DM surface, not a working feature — no WebRTC/SFU integration decision needed until this is actually picked back up.
...
12. **Aiden frontier-model providers** (Phase 14) — correction 2026-08-18: an earlier pass through this same open question misattributed the client's restated pipeline detail to "Graft and Terra." Checked against the actual code (`features/aiden/providers/`) before building anything: **VEST** and **GRAFT** were already both fully wired (`features/aiden/providers/vest-provider.ts`, `graft-provider.ts`, `registry.ts` — `productForAssetType` maps `CONCEPT_IMAGE`/`SKIN`→VEST, `TEXTURE`/`PROP`/`ANIMATION`→GRAFT, `TERRAIN`/`MAP`→TERRA). Graft is unrelated to what the client described (monument/prop/texture generation) and needed no changes. The client's restated detail was actually about:
    - **VEST's SKIN path, completed 2026-08-18.** Tripo mesh+auto-rig+baked-diffuse (already wired, `texture: true` on the Tripo task) now gets three additional Kandinsky-generated PBR maps (normal, specular/metallic-roughness, emission) and a new headless-Blender assembly stage (`features/aiden/lib/blender-assembly.ts` + `scripts/aiden/assemble_skin.py`) that wires all of it into one exported `.glb` with a real PBR material graph, instead of shipping a diffuse-only mesh. Fails soft to the pre-existing mesh-only result if Blender isn't configured on the host (`AIDEN_BLENDER_BINARY` env var) — this is a real system dependency (native binary, not an npm package), not yet installed/smoke-tested on the GoDaddy VPS this session only just learned is the target host (install script prepared: `scripts/aiden/install-blender-vps.sh`, not yet run — no SSH/VPS access from this sandbox). **Blender execution itself could not be verified in this sandbox** (no Blender binary available here) — the orchestration, argument-building, and fail-closed configuration gate are tested; the actual bpy script's 3D output correctness is not, and needs a real smoke test once Blender is installed on the VPS.
    - **The mock provider is retired for CONCEPT_IMAGE and SKIN, 2026-08-18.** Before this, `getAidenProvider()` *always* returned `MockAidenProvider` regardless of configuration — the comment literally read "No production adapter is wired in this phase," discovered while investigating this same task, not something previously documented. Every Aiden generation, for any asset type, returned a hardcoded 1×1 PNG. There were also two entirely separate, disconnected implementations: the live job pipeline (`aiden.service.ts`, `AidenJob`/`AidenAsset`, real money reservation/capture) always mocked; a second "Aiden OS" orchestration layer (`features/aiden/os/*`) that *did* call real Tripo/Replicate but had zero callers anywhere outside its own folder — fully-built, fully unreachable scaffolding. `features/aiden/lib/provider.ts#RealAidenProvider` now bridges the live pipeline onto the real `vest/graft/terra-provider.ts` registry; `getAidenProvider()` switches to it automatically once Tripo or Replicate credentials are configured (`isRealAidenProviderConfigured()`), falling back to mock only when neither vendor is configured (e.g. local dev). `AIDEN_ACTIVE_GENERATION_TYPES` gained `SKIN` alongside the pre-existing `CONCEPT_IMAGE` — before this, SKIN jobs couldn't even be *submitted* through the live product regardless of provider wiring. `completeFromProvider`'s validation/storage path, previously image-only end to end (PNG dimension reading, an 8MB image-mime-only size cap that would have rejected any mesh output outright), now branches for mesh assets (`AIDEN_MAX_MESH_BYTES`/`AIDEN_MESH_ACCEPT`), and a completed SKIN asset gets `technicalStatus: GAME_READY` instead of `CONCEPT_ONLY`. Also fixed in passing: `safeAidenFilename()` defaulted every non-JPEG/WEBP mime to a `.png` extension, which would have silently mislabeled every `.glb` mesh file as `.png`.
    - **TERRA — split into TERRAIN (shipped 2026-08-18) and MAP (still unbuilt).** Client's pipeline: Claude 3.5 Sonnet as the Logic & Geometry Engine (terrain data, heightmap matrices, object placement vectors, biome data, serialization for a playable `.map`), SDXL as the Topography & Heightmap Driver (16-bit grayscale raw terrain heightmaps, alpha splat maps, biome masks). `features/aiden/providers/terra-provider.ts` is no longer the original `TODO` stub: `TERRAIN` is now real — a single SDXL call (`generateImage({model: "SDXL_IMAGE"})`, same shape as Vest's `CONCEPT_IMAGE`/Graft's `TEXTURE`) against a grayscale-heightmap-specific prompt suffix, `isConfigured()` checks Replicate the same way Vest's concept-image path does, and `TERRAIN` was added to `AIDEN_ACTIVE_GENERATION_TYPES` so it can actually be submitted through the live product. `MAP` (the full playable-`.map` tier: Claude's structured terrain logic + multiple SDXL images assembled into one packaged deliverable) deliberately still throws a clear "not implemented yet" error and stays out of `AIDEN_ACTIVE_GENERATION_TYPES` — it doesn't fit this codebase's `generate()`-returns-one-`assetUrl` provider contract the way TERRAIN, CONCEPT_IMAGE, or TEXTURE do (Claude's output is structured text, not a hosted URL to download), and needs either a contract change or a dedicated assembly stage (the SKIN/Blender precedent) before it can ship. See `terra-provider.ts`'s doc comment for the full reasoning.
13. **KOBAads vs. Boost relationship** (Phase 15) — resolved 2026-08-18 (Phase 7): two distinct products, not one. Boost stays a purchasable ranking multiplier for organic content (fixed 10-minute/3x effect). KOBA Ads (`SponsoredCampaign`) is real paid placement — a sponsored unit that didn't organically rank there, billed CPC against a budget. They coexist; nothing merges them.
14. **KOBA Plus perks** — shipped 2026-08-15 (see Phase 16): real Stripe Subscriptions, $4.99/mo single tier, tenure badges, per-server bio. Animated avatar/banner and themes/icons/sounds are blocked on prerequisite features that don't exist at all (no avatar/banner upload, no theming system) — not just unwired. KOBA Shop cosmetic-equip access (not a discount — see open question #17) shipped with Phase 23, 2026-08-18. Multiplier perk deliberately deferred. Still open: tenure badge threshold values (placeholder), lapse-reset behavior (shipped assuming persist).
...
17. **KOBA Shop details** (Phase 23) — resolved 2026-08-18, shipped. Checkout uses a dedicated `CosmeticOrder`, not `Order`. Application review reuses the Blue-Badge staff-queue pattern. Hero is always shown to every visitor, not personalized. Client correction that landed before build (repeated again 2026-08-18 — this line hadn't been updated after the first correction): **KOBA Plus does not discount cosmetic prices.** Anyone, Plus or not, buys a cosmetic at the same price. Plus instead gates *equipping* a purchased cosmetic — `isPlusActive` checked at equip time, not at checkout. A lapsed Plus subscription auto-hides an equipped cosmetic (row stays, visibility computed at read time) without deleting the purchase.
...
_Phases 0–13 above are the original client outline and are now fully built (see README.md for ground truth), including Phase 7 (KOBA Ads, done 2026-08-18). Phases 14–24 are newer client direction, captured here as planning — status varies per phase, see each phase's own header: 16 (Plus), 17 (Server Directory + RCON), 18 (Freebies), 19 (Rarity Trading), 20 (Subdomains, MVP), 23 (KOBA Shop), and 24 (Achievements) are done; 21 (PC Plugin) and 22 (Discord Bot) have real backend pieces shipped but the actual plugin/bot artifact itself not started; 14 (Aiden) has the pipeline built and one of three model vendors (Vest/Tripo AI) actually wired, Graft/Terra still open, Reactive/Progressive Skins not built; 15 (KOBAads + Boost) is done as two separate products (see open question #13)._
