...
12. **Aiden frontier-model providers** (Phase 14) — Vest confirmed (Tripo AI, pay-as-you-go, already wired). Graft and Terra restated by the client 2026-08-18 — each is a multi-model pipeline, not a single vendor:
    - **Full Game Maps (`.map` files):** Claude 3.5 Sonnet as the Logic & Geometry Engine (terrain data, heightmap matrices, object placement vectors, biome data, serialization for a playable `.map`). SDXL as the Topography & Heightmap Driver (16-bit grayscale raw terrain heightmaps, alpha splat maps, biome masks).
    - **Full Skin Generation (mesh, rigging, skinning, textures):** Tripo Image→3D/Text→3D as the Mesh Engine (`.obj`/`.fbx` geometry for clothing/armor/items). Tripo Auto-Rig as the Skeletal & Weights Engine (bone skeletons + vertex weight influences). SDXL as the Diffuse/Albedo Driver (primary color texture). Kandinsky as the PBR Materials Engine (normal maps, specular/metallic maps, emission maps).
    - Still unconfirmed: which literal name (Graft vs. Terra) is which of the two pipelines above — doesn't block understanding what each needs to do, only naming/config once wiring starts. Neither pipeline is wired yet (the reconciliation pipeline itself is built and vendor-agnostic — this is real, substantial integration work still ahead: Claude 3.5 Sonnet terrain-logic calls, SDXL heightmap/albedo generation, the full Tripo mesh+auto-rig flow, and Kandinsky PBR map generation, each a real API integration, not yet started).
