...
12. **Aiden frontier-model providers** (Phase 14) — correction 2026-08-18: an earlier pass through this same open question misattributed the client's restated pipeline detail to "Graft and Terra." Checked against the actual code (`features/aiden/providers/`) before building anything: **VEST** and **GRAFT** were already both fully wired (`features/aiden/providers/vest-provider.ts`, `graft-provider.ts`, `registry.ts` — `productForAssetType` maps `CONCEPT_IMAGE`/`SKIN`→VEST, `TEXTURE`/`PROP`/`ANIMATION`→GRAFT, `TERRAIN`/`MAP`→TERRA). Graft is unrelated to what the client described (monument/prop/texture generation) and needed no changes. The client's restated detail was actually about:
    ...
    - **TERRA (map/terrain generation) — still genuinely unbuilt**, unchanged from before this correction. Client's pipeline: Claude 3.5 Sonnet as the Logic & Geometry Engine (terrain data, heightmap matrices, object placement vectors, biome data, serialization for a playable `.map`), SDXL as the Topography & Heightmap Driver (16-bit grayscale raw terrain heightmaps, alpha splat maps, biome masks). `features/aiden/providers/terra-provider.ts` is still the original `TODO` stub — this pipeline is real, substantial, not-yet-started work, separate from the Blender/Vest completion above.
