...
Treat these as **blocking public production** (and where noted, closed beta / real money):

1. **KOBA-SEC-001** Seed SUPERADMIN password
2. **KOBA-SEC-002** Open redirect
3. **KOBA-SEC-003** Staff MFA missing
4. **KOBA-SEC-004** Client-writable JWT `accountType`
5. **KOBA-SEC-005** Unrate-limited login
6. **KOBA-MED-001** Archive uploads without an active malware scanner
7. **KOBA-PWA-001 / KOBA-PWA-002** Private page cache + no logout purge
8. **KOBA-OPS-001** No backups / restore drill
9. **KOBA-OPS-003 / KOBA-FIN-002** No financial incident visibility
10. **KOBA-LEG-001** Missing operational policies
11. **KOBA-SEC-012** No account deletion / user suspend
12. **KOBA-DEP-001** Nested vulnerable `sharp` on an untrusted-image path
13. **KOBA-GIT-001** Cannot launch from current `origin/main`
14. **KOBA-FIN-003** (real-money auctions) Request-driven settlement

Not proven, therefore **not** listed as blockers: ledger invariant *failure*, unrestricted SSRF, exposed production Postgres/Redis, committed live credentials.
...
A staging VPS *can* run this commit for internal QA with:
...
Blockers: staff MFA, seed password, open redirect, login brute force, JWT claim spoof, legal pages, backups, no user suspend/deletion, observability. Invite-only users still deserve those controls if they can pay, message, or upload.
...
All closed-beta blockers, plus PWA cache, malware hook, `sharp` advisory, and integration-branch gap vs `origin/main`.
...
*End of Phase 15A audit. No fixes, commits, pushes, or production contact were performed.*
